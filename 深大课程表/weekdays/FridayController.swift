...
class FridayController : UIViewController,UITableViewDelegate,UITableViewDataSource,UIAlertViewDelegate {
                ...
                while(i<7){
                    ...
                    if (weekday == 5){
                        ...
                        for j in kecheng{
                            ...
                            Fridaytime.insert("\(kecheng_begin)~\(kecheng_end)", atIndex: p)
                            Fridaylocal.insert(kecheng_locale, atIndex: p)
                            Fridaycourse.insert(kecheng_name, atIndex: p)
                            p++
                        }
                    }
    ...
    func setuptable(){
        ...
        coursecount.text = "\(Fridaycourse.count)"
    ...
    func tableView(tableView: UITableView, cellForRowAtIndexPath indexPath: NSIndexPath) -> UITableViewCell {
        ...
        if Fridaycourse.count < 6{
            if(indexPath.row < Fridaycourse.count)
            {
                label.text = Fridaycourse[indexPath.row]
    ...
    func tableView(tableView: UITableView, didSelectRowAtIndexPath indexPath: NSIndexPath) {
        ...
        if Fridaycourse.count < 6{
            if(indexPath.row < Fridaycourse.count)
            {
                var view : UIAlertView = UIAlertView(title: Fridaycourse[indexPath.row], message: "\(Fridaytime[indexPath.row])在\(Fridaylocal[indexPath.row])上课", delegate: self, cancelButtonTitle: "我知道了")
                view.show()
            }
            else{
