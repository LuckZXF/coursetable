...
class MondayController : UIViewController,UITableViewDelegate,UITableViewDataSource {
  //  @IBOutlet weak var webView: UIWebView!
                ...
                while(i<7){
                    ...
                    if (weekday == 1){
                        ...
                        for j in kecheng{
                            ...
                            Mondaytime.insert("\(kecheng_begin)~\(kecheng_end)", atIndex: p)
                            Mondaylocal.insert(kecheng_locale, atIndex: p)
                            Mondaycourse.insert(kecheng_name, atIndex: p)
                            p++
                        }
                    }
    ...
    func tableView(tableView: UITableView, cellForRowAtIndexPath indexPath: NSIndexPath) -> UITableViewCell {
       ...
       // cell.textLabel?.text = "岗位实践(4)"
        ...
        if Mondaycourse.count < 6{
            if(indexPath.row < Mondaycourse.count)
            {
                label.text = Mondaycourse[indexPath.row]
            }
            else{
                label.text = ""
            }
        }
        else if Mondaycourse.count >= 6{
            label.text = Mondaycourse[indexPath.row]
        }
    ...
    func tableView(tableView: UITableView, didSelectRowAtIndexPath indexPath: NSIndexPath) {
        ...
        if Mondaycourse.count < 6{
            if(indexPath.row < Mondaycourse.count)
            {
                var view : UIAlertView = UIAlertView(title: Mondaycourse[indexPath.row], message: "\(Mondaycourse[indexPath.row])在\(Mondaycourse[indexPath.row])上课", delegate: self, cancelButtonTitle: "我知道了")
                view.show()
            }
            else{
