...
class TuesdayController : UIViewController,UITableViewDelegate,UITableViewDataSource,UIAlertViewDelegate {
                ...
                while(i<7){
                    ...
                    if (weekday == 2){
                        ...
                        for j in kecheng{
                            ...
                            Tuesdaytime.insert("\(kecheng_begin)~\(kecheng_end)", atIndex: p)
                            Tuesdaylocal.insert(kecheng_locale, atIndex: p)
                            Tuesdaycourse.insert(kecheng_name, atIndex: p)
                            p++
                        }
                    }
    ...
    func setuptable(){
        ...
        coursecount.text = "\(Tuesdaycourse.count)"
    ...
    func tableView(tableView: UITableView, cellForRowAtIndexPath indexPath: NSIndexPath) -> UITableViewCell {
        ...
        if Tuesdaycourse.count < 6{
            if(indexPath.row < Tuesdaycourse.count)
            {
                label.text = Tuesdaycourse[indexPath.row]
    ...
    func tableView(tableView: UITableView, didSelectRowAtIndexPath indexPath: NSIndexPath) {
        ...
        if Tuesdaycourse.count < 6{
            if(indexPath.row < Tuesdaycourse.count)
            {
                var view : UIAlertView = UIAlertView(title: Tuesdaycourse[indexPath.row], message: "\(Tuesdaytime[indexPath.row])在\(Tuesdaylocal[indexPath.row])上课", delegate: self, cancelButtonTitle: "我知道了")
                view.show()
            }
            else{
