...
class ThursdayController : UIViewController,UITableViewDelegate,UITableViewDataSource,UIAlertViewDelegate {
                ...
                while(i<7){
                    ...
                    if (weekday == 4){
                        ...
                        for j in kecheng{
                            ...
                            Thursdaytime.insert("\(kecheng_begin)~\(kecheng_end)", atIndex: p)
                            Thursdaylocal.insert(kecheng_locale, atIndex: p)
                            Thursdaycourse.insert(kecheng_name, atIndex: p)
                            p++
                        }
                    }
    ...
    func setuptable(){
        ...
        coursecount.text = "\(Thursdaycourse.count)"
    ...
    func tableView(tableView: UITableView, cellForRowAtIndexPath indexPath: NSIndexPath) -> UITableViewCell {
        ...
        if Thursdaycourse.count < 6{
            if(indexPath.row < Thursdaycourse.count)
            {
                label.text = Thursdaycourse[indexPath.row]
                // print(Tuesdaycourse[indexPath.row])
            }
            else{
                label.text = ""
            }
        }
        else if Thursdaycourse.count >= 6{
            label.text = Thursdaycourse[indexPath.row]
        }
    ...
    func tableView(tableView: UITableView, didSelectRowAtIndexPath indexPath: NSIndexPath) {
        ...
        if Thursdaycourse.count < 6{
            if(indexPath.row < Thursdaycourse.count)
            {
                var view : UIAlertView = UIAlertView(title: Thursdaycourse[indexPath.row], message: "\(Thursdaytime[indexPath.row])在\(Thursdaylocal[indexPath.row])上课", delegate: self, cancelButtonTitle: "我知道了")
                view.show()
            }
            else{
            ...
            var view : UIAlertView = UIAlertView(title: Tuesdaycourse[indexPath.row], message: "\(Thursdaytime[indexPath.row])在\(Thursdaylocal[indexPath.row])上课", delegate: self, cancelButtonTitle: "我知道了")
            view.show()
        }
    }
}
